...
FireSync uses [JSON Web Tokens (JWTs)](https://en.wikipedia.org/wiki/JSON_Web_Token) for authentication. To grant a user access to FireSync documents you should create a JWT which specifies the documents the user can access. The JWT is signed by a secret that only you and FireSync know so FireSync can confirm the user is authorized by you. You should create and sign the JWT on your backend where the shared secret is available, then pass the JWT down to the user's client where it can be included with requests to FireSync.
